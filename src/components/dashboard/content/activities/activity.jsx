...
import { BiFilter, BiNote, BiPlus, BiTrash, BiArrowToLeft, BiArrowToRight, BiEdit } from 'react-icons/bi';
...
export const Activity = () => {
  ...
  const confirmDeleteMulti = async (activitiesIDs) => {
    // const deletePromises = activitiesIDs.map((id) => {
    //   return activitiesServiceApi.deleteActivities({ data: { idActivities: activitiesID } });
    // });
  ...
  const handleSelectAllCheckbox = () => {
    ...
    setSelectedActivity(prev => selectAllCheckbox ? [] : activities.map(activity => activity._id));
  };
              ...
              {showCheckboxes && (
                <td className='td-select'>
                  <input
                    className='ck-box'
                    type="checkbox"
                    checked={selectedActivity.includes(activity._id)}
                    onChange={() => handleActivityCheckboxToggle(activity._id, getSubActivityForActivity(activity._id))}
                  />
                </td>
              )}
              <td>{activity.name}</td>
              <td>{activity.description}</td>
              <td>{activity.isParentType}</td>
              <td>
                {subActivities
                  .filter((subActivity) => subActivity.idActivities === activity._id)
                  .map((subActivity) => (
                    <React.Fragment key={subActivity._id}>
                      {subActivity.content}
